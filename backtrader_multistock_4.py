...
class SMAC(bt.Strategy):
    ...
    def __init__(self):
        ...
        # if self.params.ema1 > self.params.ema2:
        #     raise ValueError(
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                trade_dictionary = {"time": self.datas[0].datetime.datetime(0), "stock": order.data._name,
                                    "qty": order.size,
...
if __name__ == '__main__':
    ...
    symbols = ['adaniports_15min.csv', 'asianpaint_15min.csv', 'axisbank_15min.csv']
    ...
    for s in symbols:
        data = bt.feeds.GenericCSVData(dataname="./data/" + s,
    ...
    for strat in res:
        ...
        stats_summary = {"run_name": name_of_run, "end": str(retur_perc['end']), "growth": str(retur_perc['growth']),
                         "return": str(retur_perc['return']), "sharpe": str(sharpe['sharperatio']),
                         "SQN": str(sq["sqn"]),
