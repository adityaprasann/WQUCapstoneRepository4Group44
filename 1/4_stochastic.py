...
class SMAC(bt.Strategy):
    """A simple moving average crossover strategy; crossing of a fast and slow moving average generates buy/sell
       signals"""
    params = {"sma": 9,"trix": 3, "stochastic": 3, "variable1": 15 , "variable2":10 ,# The windows for both fast and slow moving averages
              "optim": False, "optim_variable": (9,3,3,15,10)}  # Used for optimization; equivalent of fast and slow, but a tuple
    def __init__(self):
        ...
        if self.params.optim:  # Use a tuple during optimization
            self.params.sma,self.params.trix,self.params.stochastic,self.params.variable1,self.params.variable2 = self.params.optim_variable  # fast and slow replaced by tuple's contents
        ...
        for d in self.getdatanames():  # Looping through all symbols
            ...
            if pos == 0:  # Are we out of the market?
                ...
                if self.trix[d][0] > 0  and self.stochastic[d][0] < self.params.variable1 and self.dataclose[d][0]>self.dataopen[d][0] :# A buy signal
                   # and self.dataclose[d][-1] > self.dataopen[d][-1] and self.dataclose[d][-2] > self.dataopen[d][-2]
                ...
                if self.trix[d][0] < 0 and self.stochastic[d][0] > 100-self.params.variable2 and self.dataopen[d][0]>self.dataclose[d][0] and\
                        self.dataopen[d][-1]>self.dataclose[d][-1]  :  # A sell signal and self.datahigh[d][0]<self.dataclose[d][-1]
